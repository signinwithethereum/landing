...
Same arguments and verification as [`#verify`](#verify), but raises the [`Siwe::Error`](#error-handling) on failure and returns `self` on success — the idiomatic path for a Rails / Sinatra controller:
